...
def open_transfers_df(transfers_df_path):
    transfers_df = pd.read_csv(transfers_df_path, sep = ",", skiprows=1, header = None, names = ["Donor", "Receiver", "Frequency", "Gene_Index"])
    return transfers_df
...
def translate_tree(sampled_tree, translation_df):
    ...
    for node in sampled_tree.traverse():
        try:
            ...
            node.add_features(ale_name=name_dict[node.name])
        except KeyError:
...
def determine_descendant_relations(tree, df):
    ...
    def get_descendants(node_name):
        if node_name not in descendants_cache:
            node = tree.search_nodes(ale_name=node_name)[0]  # Assume each name uniquely identifies a node
